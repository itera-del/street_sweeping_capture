...
def split(args):
    ...
    with open(args.to_all_file, "w") as f:

        for _, row in tqdm(data_pd.iterrows(), total=len(data_pd)):
            if row['split_type'] == "train" or row['split_type'] == "val" or row['split_type'] == "test":
                f.write('{}'.format(row["roi_img_path"]))
                f.write("\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ocr_name', type=str, default="plate_chile_202312") 
    ...
    args.to_all_file = os.path.join(args.input_dir, "ImageSets_aug/ImageSets/Main/all.txt")
