...
def kind_num_city_mask_2_bbox(mask, dataset_dict, kind_min_size_threh, num_min_size_threh, city_min_size_threh):
    
    seg_bbox = {}

    for key in dataset_dict.class_seg_label_group_2_id_map.keys():

        if key != "kind" and key != "num" and key != "city":
            continue
        ...
        if label_size > 0.0:
            ...
            if key == "kind":
                ...
                if w >= city_min_size_threh and h >= city_min_size_threh:
                    seg_bbox[key] = [[label_bbox[0], label_bbox[1], w, h]]

    return seg_bbox


def get_resize_shape(kind_img, num_img, city_img):
