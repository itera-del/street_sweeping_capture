...
def analysis_dataset_label(args):
    ...
    for _, row in tqdm(data_pd.iterrows(), total=len(data_pd)):
        ...
        # analysis info
        analysis_info = {}
        analysis_info["country"] = plate_country
        analysis_info["city"] = plate_city
        analysis_info["color"] = plate_color
        analysis_info["column"] = plate_column
        analysis_info["num"] = plate_num
        analysis_info["kind"] = plate_num
        ...
        # kind
        key_name = "kind"
        plate_num = analysis_info[key_name]
        if "#" in plate_num:
            num = plate_num.split("#")
        else:
            num = [plate_num]
    ...
    key_name = "kind"
    out_csv_path = os.path.join(args.output_analysis_dir, 'analysis_{}.csv'.format(key_name))
    analysis_pd = pd.DataFrame(analysis_dict[key_name], index=[args.date_name])
    analysis_pd.to_csv(out_csv_path, encoding="utf_8_sig")
    
    # info 
    info_num = 0    
    for info_key_name in args.info_list:
        if info_key_name in analysis_dict[key_name]:
            info_num += analysis_dict[key_name][info_key_name]
    print("{}: {}".format(args.info_list, info_num))
    print("total: {}".format(len(data_pd)))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--date_name', type=str, default="uae_20241115") 
    parser.add_argument('--input_csv_dir', type=str, default="/yuanhuan/data/image/RM_ANPR/original/zd/UAE/UAE_csv/") 
    parser.add_argument('--output_analysis_dir', type=str, default="/yuanhuan/data/image/RM_ANPR/original/zd/UAE/UAE_analysis/") 
