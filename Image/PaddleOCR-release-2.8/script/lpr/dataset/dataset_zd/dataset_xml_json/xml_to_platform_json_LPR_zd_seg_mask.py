...
def xml_2_platform_json(args):
    ...
    for idx in tqdm(range(len(xml_list))):
        ...
        for object in root.findall('object'):
            ...
            # color
            license_color = "unknown"
            # country
            # country_f
            license_country = "unknown"
            license_country_f = "unknown"
            # city
            # city_f
            license_city = "unknown"
            license_city_f = "unknown"
            # car_type
            # car_type_f
            license_car_type = "unknown"
            license_car_type_f = "unknown"

            # color
            if classname in color_name_list[1:]:                
                if classname in ['red', 'green', 'yellow', 'blue']:
                    license_color = classname
                    bndbox.extend(["license_color"])
                    bndbox.extend([[{"name": "license_color", "value": license_color}]])
            
            # country
            # country_f
            elif classname in country_name_list[1:]:
                if classname in ['uae', 'ksa']:
                    license_country = classname
                    bndbox.extend(["license_country"])
                    bndbox.extend([[{"name": "license_country", "value": license_country}]])
                elif classname in ['oman']:
                    license_country_f = classname + '_f'
                    bndbox.extend(["license_country_f"])
                    bndbox.extend([[{"name": "license_country_f", "value": license_country_f}]])
            elif classname in country_f_name_list[1:]:
                if classname in ['uae_f', 'oman_f']:
                    license_country_f = classname
                    bndbox.extend(["license_country_f"])
                    bndbox.extend([[{"name": "license_country_f", "value": license_country_f}]])

            # city
            # city_f
            elif classname in city_name_list[1:]:
                if classname in ['ad', 'abudhabi', 'dubai', 'ajman', 'sharjah', 'shj', 'rak']:
                    license_city = classname
                    bndbox.extend(["license_city"])
                    bndbox.extend([[{"name": "license_city", "value": license_city}]])
                elif classname in ['ummalqaiwain', 'fujairah']:
                    license_city_f = classname + '_f'
                    bndbox.extend(["license_city_f"])
                    bndbox.extend([[{"name": "license_city_f", "value": license_city_f}]])
            elif classname in city_f_name_list[1:]:
                if classname in ['ad_f', 'abudhabi_f', 'dubai_f', 'ajman_f', 'sharjah_f', 'shj_f', 'rak_f', 'ummalqaiwain_f', 'fujairah_f']:
                    license_city_f = classname
                    bndbox.extend(["license_city_f"])
                    bndbox.extend([[{"name": "license_city_f", "value": license_city_f}]])

            # car_type
            # car_type_f
            elif classname in car_type_name_list[1:]:
                if classname in ['taxi', 'police', 'trp', 'ptr']:
                    license_car_type = classname
                    bndbox.extend(["license_car_type"])
                    bndbox.extend([[{"name": "license_car_type", "value": license_car_type}]])
                elif classname in ['trade']:
                    license_car_type_f = classname + '_f'
                    bndbox.extend(["license_car_type_f"])
                    bndbox.extend([[{"name": "license_car_type_f", "value": license_car_type_f}]])
            elif classname in car_type_f_name_list[1:]:
                if classname in ['taxi_f', 'police_f', 'trp_f', 'ptr_f', 'trade_f']:
                    license_car_type_f = classname
                    bndbox.extend(["license_car_type_f"])
                    bndbox.extend([[{"name": "license_car_type_f", "value": license_car_type_f}]])
