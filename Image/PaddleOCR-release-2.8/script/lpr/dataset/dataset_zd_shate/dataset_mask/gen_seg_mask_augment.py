...
def gen_seg_mask_augument(args):
    ...
    # dataset_zd_shate_dict
    # city
    dataset_city_dict = importlib.import_module('script.lpr.dataset.dataset_zd_shate.dataset_dict.' + args.seg_city_dict_name) 
    # color
    dataset_color_dict = importlib.import_module('script.lpr.dataset.dataset_zd_shate.dataset_dict.' + args.seg_color_dict_name) 
    ...
    for idx, row in tqdm(data_pd.iterrows(), total=len(data_pd)):
        ...
        # object_roi_list
        object_city_roi_list = dataset_city_dict.load_object_roi(crop_xml_path, crop_json_path, args.new_style)
        object_color_roi_list = dataset_color_dict.load_object_roi(crop_xml_path, crop_json_path, args.new_style)
        object_roi_list = object_city_roi_list
        ...
        for idy in range(args.aug_times):
            ...
            csv_city_list.append({"img_path": img_path, "json_path": json_path, "roi_img_path": output_city_img_path, "roi_mask_path": output_city_mask_path, "id": plate_id, "name": plate_name, "roi": plate_roi, "country": plate_country, "color": plate_color, "column": plate_column, "num": plate_num, "crop_img": crop_img_path, "crop_xml": crop_xml_path, "crop_json": crop_json_path, "split_type": split_type})
            csv_color_list.append({"img_path": img_path, "json_path": json_path, "roi_img_path": output_color_img_path, "roi_mask_path": output_color_mask_path, "id": plate_id, "name": plate_name, "roi": plate_roi, "country": plate_country, "color": plate_color, "column": plate_column, "num": plate_num, "crop_img": crop_img_path, "crop_xml": crop_xml_path, "crop_json": crop_json_path, "split_type": split_type})
...
if __name__ == "__main__":
    ...
    parser.add_argument('--date_name', type=str, default="test") 
    ...
    ###############################################
    # dataset_zd_shate_dict & dataset_zd_shate_dict_color
    ###############################################
