...
def get_kind_num_id(kind_num_name):
    """
    从车牌名称中提取种类ID（英文字母）和数字ID（全数字）
    
    Args:
        kind_num_name (str): 车牌名称，可能包含#分隔符
        
    Returns:
        tuple: (kind_id, num_id) - 种类ID和数字ID
    """
    # 初始化返回值
    kind_id, num_id = '', ''
    ...
    if len(img_name_list) == 1:
        # 只有一个部分时，判断是字母还是数字
        if img_name_list[0].isalpha():
            kind_id = img_name_list[0]
        elif img_name_list[0].isdigit():
            num_id = img_name_list[0]
    elif len(img_name_list) == 2:
        # 有两个部分时，分别判断每个部分的类型
        part1, part2 = img_name_list[0], img_name_list[1]
