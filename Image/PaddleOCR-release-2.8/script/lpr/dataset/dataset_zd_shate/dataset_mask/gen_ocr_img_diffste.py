...
def gen_img(args, dataset_dict):
    ...
    for _, row in tqdm(args.data_pd.iterrows(), total=len(args.data_pd)):
        ...
        for idx in tqdm(range(len(diffste_img_list))):
            ...
            csv_list.append({"img_path": img_path, "json_path": json_path, "roi_img_path": output_img_path, "id": plate_id, "name": plate_name, "roi": plate_roi, "country": plate_country, "color": plate_color, "column": plate_column, "kind": kind_id, "num": num_id, "crop_img": crop_img_path, "crop_xml": crop_xml_path, "crop_json": crop_json_path, "split_type": split_type})
...
def gen_ocr_img(args):
    ...
    # dataset_zd_dict
    dataset_dict = importlib.import_module('script.lpr.dataset.dataset_zd_shate.dataset_dict.' + args.seg_dict_name) 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--seg_name', type=str, default="seg_shate_202508")
    parser.add_argument('--ocr_name', type=str, default="plate_shate_mask_202508")
    parser.add_argument('--output_dir', type=str, default="/yuanhuan/data/image/RM_ANPR/training/") 
    parser.add_argument('--diffste_name', type=str, default="diffste_2094_shate") 
    parser.add_argument('--diffste_dir', type=str, default="/yuanhuan/data/image/RM_ANPR/original/zd/Shate/Shate_crop_scale_padding/DIFFSTE/") 
