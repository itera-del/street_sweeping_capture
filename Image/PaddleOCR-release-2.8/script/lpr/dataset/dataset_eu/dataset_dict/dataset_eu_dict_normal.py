...
def json_load_object_plate_status(cell):
    ...
    if not bool_find_status:
        label = cell["label"]
        
        if label in ['cover-motorplate', 'cover-plaet']:
            plate_status = status_name_list[2]
        elif label in ['fuzzy-motorplate', 'fuzzy-plate']:
            plate_status = status_name_list[1]
        elif label in ['lince-motorplate', 'lince-plate']:
            plate_status = status_name_list[0]
...
def json_load_object_plate_column(cell, w, h):
    ...
    if not bool_find_column:
        label = cell["label"]
        
        if label in ['lince-plate', 'cover-plaet', 'fuzzy-plate']:
            plate_column = column_name_list[2]
        elif label in ['lince-motorplate', 'cover-motorplate', 'fuzzy-motorplate']:
            plate_column = column_name_list[1]
