...
class CityMultiLabelSegTrainingEngine():

    def __init__(self, img_list_path, batch_size, seg_dict_name='dataset_zd_dict_multi_label', inputSize=[3,128,64], save_path='./output/seg_city/'):

        # dataset_zd_dict
        self.dataset_dict = importlib.import_module('script.dataset.dataset_seg_zd.dataset_dict.' + seg_dict_name) 

        self.seg_labels = self.dataset_dict.class_seg_sigmoid_label
        ...
        valdata = segdata.Platedataset(img_list_path, "val", transform_test, mask_type="pkl")
        testdata = segdata.Platedataset(img_list_path, "test", transform_test, mask_type="pkl")
        traindata = segdata.Platedataset(img_list_path, "train", transform_train, mask_type="pkl")
    ...
    def train(self, epoch):
        ...
        for batch_idx, sample in tqdm(enumerate(self.Traindataloader), total=len(self.Traindataloader)):
            ...
            seg_pred = self.model(img, seg_label)
    ...
    def val(self, epoch):
        ...
        for batch_idx, sample in tqdm(enumerate(self.Valdataloader),total = len(self.Valdataloader)):
            ...
            loss = self.criterion_rmi(seg_pred, seg_label)
            if isinstance(self.model, torch.nn.DataParallel):
                loss = loss.sum()
            val_loss += loss.item()
    ...
    def test(self, epoch):
        ...
        for batch_idx, sample in tqdm(enumerate(self.Testdataloader), total=len(self.Testdataloader)):
            ...
            seg_label = sample['segLabel'].to(device)
            seg_pred = self.model(img, seg_label)
            seg_pred_sigmoid = seg_pred.sigmoid()
            seg_pred_sigmoid = seg_pred_sigmoid.detach().cpu().numpy()
            ...
            for idx in range(len(img_cpu)):
                ...
                # mask
                seg_pred_idx = seg_pred_sigmoid[idx]
                mask_img = np.zeros(img.shape, dtype='int64')

                thread = 0.5
                for chn_idx in range(len(self.seg_labels)):

                    mask_img[seg_pred_idx[chn_idx] > thread] += self.dataset_dict.id_2_sigmoid_mask_color_dict[chn_idx]
