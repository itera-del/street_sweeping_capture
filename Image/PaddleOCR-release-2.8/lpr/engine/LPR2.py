...
class TrainingEngine():
    ...
    def __init__(self, imglistPath, imgtestlistPath, batchSize, inputSize=[1,64,300], savepath="./output/mexico/"):
        ad_labels = ['%', '1', '2', '3', '4', '5', '6', '7', '8', '9', '0', 'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I',
              'J', 'K', 'L', 'M', 'N', 'P', 'O', 'Q', 'R', 'S', 'T', 'U', 'V', 'W', 'X', 'Y', 'Z','-']
        self.early_stop = 500
        nclass = len(ad_labels)
        self.best_accuracy = 0.1
        self.savepath = savepath
        if not os.path.exists(savepath):
            os.mkdir(savepath)
        self.converter=dubaiplate.Str2Label(ad_labels)
        ...
        traindata = dubaiplate.DatasetPlate3(imglistPath,inputSize=[1,64,300],istrain=1)
        testdata = dubaiplate.DatasetPlate3(imgtestlistPath,inputSize=[1,64,300],istrain=0)
        self.Traindataloader = DataLoader(traindata, batch_size=batchSize, shuffle=True, num_workers=4)
        self.Valdataloader = DataLoader(testdata, batch_size=batchSize, shuffle=True, num_workers=4)
        self.optimizer = torch.optim.Adam(self.crnn.parameters(), lr=0.00005,betas=(0.5, 0.999))
