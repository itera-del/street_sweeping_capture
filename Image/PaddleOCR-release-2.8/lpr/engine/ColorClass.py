...
class ColorClasTrainingEngine():
    
    def __init__(self, img_list_path, batch_size, seg_color_dict_name='dataset_cn_dict_color', inputSize=[3,128,64], save_path='./output/seg_city/', bool_no_grad=False):

        # dataset_cn_dict
        self.dataset_color_dict = importlib.import_module('script.lpr.dataset.dataset_cn.dataset_dict.' + seg_color_dict_name) 
        
        self.seg_color_labels = self.dataset_color_dict.class_seg_label

        self.save_path = save_path
        self.batch_size = batch_size
        self.bool_no_grad = bool_no_grad
        ...
        # model
        self.model = NovaLaneNet.LANENET_NOVT_HEAD_SEG_CLA(num_classes_1_head=1, num_classes_2_head=len(self.seg_color_labels), bool_no_grad=self.bool_no_grad).cuda()
        ...
        valdata = segdata.Platedataset_HEAD_CLA(img_list_path, "val", transform_test)
        testdata = segdata.Platedataset_HEAD_CLA(img_list_path, "test", transform_test)
        traindata = segdata.Platedataset_HEAD_CLA(img_list_path, "train", transform_train)
    ...
    def train(self, epoch):
        ...
        for batch_idx, sample in tqdm(enumerate(self.Traindataloader), total=len(self.Traindataloader)):
            ...
            seg_label = sample['segLabel_cla'].to(device)

            self.optimizer.zero_grad()
            seg_pred_1, seg_pred_2 = self.model(img)

            loss3 = self.criterion_color(seg_pred_2, seg_label)
            loss_color = loss3
            loss = loss_color
            ...
            if iter_idx % 200==0:
                print("batch loss: {:.3f}, loss_color: {:.3f}".format(loss.item(), loss_color.item()), batch_idx, len(self.Traindataloader)/self.batch_size)
        ...
        if epoch % 1 == 0:
            ...
            torch.save(save_dict, self.model_name.replace('.pth', '_{}.pth'.format(epoch)))
    ...
    def val(self, epoch):
        ...
        for batch_idx, sample in tqdm(enumerate(self.Valdataloader),total = len(self.Valdataloader)):
            ...
            loss_color = self.criterion_color(seg_pred_2, seg_label)
            loss = loss_color
            if isinstance(self.model, torch.nn.DataParallel):
                loss = loss.sum()
                loss_color = loss_color.sum()
            ...
            for idx in range(len(img_cpu)):
                ...
                for chn_idx in range(len(self.seg_color_labels)):
                    ...
                    if seg_make_2.max() == chn_idx:
                        ...
                        res_color = self.dataset_color_dict.id_2_mask_name_dict[chn_idx]

                mask_img = mask_img.astype(img.dtype)
                img_save = cv2.addWeighted(src1=img, alpha=0.8, src2=mask_img, beta=1.0, gamma=0.)
                # cv2.imwrite(self.save_path+"/result/"+str(num) + "_" + res_color + ".jpg", img_save) 
                cv2.imwrite(self.save_path+"/result/"+str(num) + "_" + res_color + ".jpg", img) 
    ...
    def __call__(self):
        ...
        # load model
        try:
            self.LoadPretrainModel(self.best_model_name)
