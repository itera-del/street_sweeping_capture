...
class CityColorSegTrainingEngine():

    def __init__(self, img_list_path, batch_size, seg_city_dict_name='dataset_zd_dict', seg_color_dict_name='dataset_zd_dict_color', inputSize=[3,128,64], save_path='./output/seg_city/'):

        # dataset_zd_dict
        self.dataset_city_dict = importlib.import_module('script.dataset.dataset_seg_zd.dataset_dict.' + seg_city_dict_name) 
        self.dataset_color_dict = importlib.import_module('script.dataset.dataset_seg_zd.dataset_dict.' + seg_color_dict_name) 
        ...
        # model
        self.model = NovaLaneNet.LANENET_NOVT_2HEAD(num_classes_1_head=len(self.seg_city_labels), num_classes_2_head=len(self.seg_color_labels)).cuda()
        ...
        self.criterion_rmi_color = RMILoss(num_classes=len(self.seg_color_labels)).to(device)
        ...
        valdata = segdata.Platedataset_2Head(img_list_path, "val", transform_test)
        testdata = segdata.Platedataset_2Head(img_list_path, "test", transform_test)
        traindata = segdata.Platedataset_2Head(img_list_path, "train", transform_train)
    ...
    def train(self, epoch):
        ...
        for batch_idx, sample in tqdm(enumerate(self.Traindataloader), total=len(self.Traindataloader)):
            ...
            seg_label_2 = sample['segLabel_2'].to(device)

            self.optimizer.zero_grad()
            seg_pred_1, seg_pred_2 = self.model(img)

            loss1 = self.criterion_city(seg_pred_1, seg_label_1)
            loss2 = self.criterion_rmi_city(seg_pred_1, seg_label_1)
            loss3 = self.criterion_color(seg_pred_2, seg_label_2)
            loss4 = self.criterion_rmi_color(seg_pred_2, seg_label_2)
            loss_city = loss1 + loss2
            loss_color = loss3 + loss4
            # loss_color = loss3
            loss = loss_city + 0.1 * loss_color
            if isinstance(self.model, torch.nn.DataParallel):
                loss = loss.sum()
    ...
    def val(self, epoch):
        ...
        for batch_idx, sample in tqdm(enumerate(self.Valdataloader),total = len(self.Valdataloader)):
            ...
            loss_color = self.criterion_color(seg_pred_2, seg_label_2)
            loss = loss_city + 0.1 * loss_color
            if isinstance(self.model, torch.nn.DataParallel):
                loss = loss.sum()
                loss_city = loss_city.sum()
        ...
        for batch_idx, sample in tqdm(enumerate(self.Testdataloader), total=len(self.Testdataloader)):
            ...
            for idx in range(len(img_cpu)):
                ...
                for chn_idx in range(len(self.seg_color_labels)):
                    ...
                    mask_img[seg_make_2 == chn_idx] += self.dataset_color_dict.id_2_mask_color_dict[chn_idx]
