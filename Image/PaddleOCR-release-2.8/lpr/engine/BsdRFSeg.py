...
class BsdSegTrainingEngine():
    ...
    def __init__(self, imglistPath, batchSize, inputSize=[3,256,128], savepath='./output/bsdrf/'):
        self.early_stop = 50
        self.best_loss = 8
        ...
        if not os.path.exists(savepath):
            os.mkdir(savepath)
        self.transform_img = Resize((256, 128))
        ...
        weight=torch.tensor([2.0, 4,4,2,2,2])
        testweight=torch.tensor([2.0, 4,4,2,2,2])
        #self.model = LaneNet.LANENet(num_classes=5).cuda()
        self.model = NovaLaneNet.LANENET_NOVT_TEST(num_classes=6).cuda()
        self.criterion = torch.nn.CrossEntropyLoss(weight=weight).to(device)
        self.criterion_rmi = RMILoss(num_classes=6).to(device)
        ...
        valdata = segdata.BsdSegRFData(imglistPath,"val",transform_test)
        testdata = segdata.BsdSegRFData(imglistPath,"test",transform_test)
        traindata = segdata.BsdSegRFData(imglistPath,"train", transform_train)
        ...
        self.Traindataloader = DataLoader(traindata, batch_size=batchSize, shuffle=True, collate_fn=traindata.collate, num_workers=8)
    ...
    def test_video(self, videopath):
        ...
        for i, v in enumerate(vid_list):
            ...
            video_writer = cv2.VideoWriter(self.savepath + '/'+str(i)+".mp4", cv2.VideoWriter_fourcc(*'XVID'), fps, (wid, hei))
            for ii in tqdm(range(num)):
                (flag, image) = video_reader.read()
                mask = np.zeros_like(image)
                imageCrop = image#[0:1080,300:1500,:]
                maskCrop = mask#[0:1080,300:1500,:]
                ...
                coord_mask = cv2.resize(coord_mask, dsize=(1280, 720), interpolation=cv2.INTER_NEAREST)
                #coord_mask = cv2.resize(coord_mask, dsize=(1920, 1080), interpolation=cv2.INTER_NEAREST)
    ...
    def __call__(self):
        ...
        while Iteration < 200:
            ...
            if Iteration % 50 == 0:
                self.test(Iteration)
                self.test_video('./input/dataset_bsdrfseg/video')
            #self.test(Iteration)  
    ...
    def __call__C(self):
        self.LoadPretrainModel(self.model_name)
        print('load pretrain model successfully!')
        #self.test_video("./input/dataset_bsdrfseg/video/")#input\dataset_bsdrfseg\video
        self.testcaffe()
