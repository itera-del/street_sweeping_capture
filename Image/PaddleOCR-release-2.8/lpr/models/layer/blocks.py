...
class InitialBlock(nn.Module):
    ...
    def __init__(self,
        ...
        # Extension branch
        self.ext_branch = nn.MaxPool2d(kernel_size=2, stride=2, ceil_mode=True)
...
class DownsamplingBottleneck2(nn.Module):
    """Downsampling bottlenecks further downsample the feature map size.
    Main branch:
    1. max pooling with stride 2; indices are saved to be used for
    unpooling later.
...
class UpsamplingBottleneck(nn.Module):
    ...
    def __init__(self,
        ...
        self.main_unpool1 = nn.MaxUnpool2d(kernel_size=2, stride=2)
...
class UpsamplingBottleneck2(nn.Module):
    """The upsampling bottlenecks upsample the feature map resolution using max
    pooling indices stored from the corresponding downsampling bottleneck.
    Main branch:
    1. 1x1 convolution with stride 1 that decreases the number of channels by
    ...
    def __init__(self,
        ...
        # Remember that the stride is the same as the kernel_size, just like
        # the max pooling layers
        self.main_ConvTranspose2d = nn.ConvTranspose2d(
                internal_channels,
                internal_channels,
                kernel_size=2,  # 3kernel_size
                stride=2,
    ...
    def forward(self, x):
        ...
        # main = self.main_conv1(x)
        main = self.main_conv1(x)
        main = self.main_ConvTranspose2d(main)
        # Extension branch
        ext = self.ext_conv1(x)
        ext = self.ext_conv2(ext)
        ext = self.ext_conv3(ext)
...
class UpsamplingBottleneck3(nn.Module):
    """The upsampling bottlenecks upsample the feature map resolution using max
    pooling indices stored from the corresponding downsampling bottleneck.
    Main branch:
    1. 1x1 convolution with stride 1 that decreases the number of channels by
