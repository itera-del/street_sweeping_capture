...
__all__ = ['Conv2dBatchLeaky', 'Conv2dBatch', 'GlobalAvgPool2d', 'PaddedMaxPool2d', 'Reorg', 'SELayer',
            ...
            'Conv2dBatchPReLU', 'DilatedConv2dBatchReLU','Conv2dBatchPLU', 'Conv2dBatchELU', 'Conv2dBatchSELU',
            'Shuffle', 'Conv2dBatchReLU', 'FullyConnectLayer', 'Conv2d1x1', 'Conv2dDepthWise', 'buildInvertedResBlock',
            'layerConcat', 'Conv2d1x1Relu6', 'hourGlass1', 'Conv2d']
...
class DilatedConv2dBatchReLU(nn.Module):
    """ This convenience layer groups a 2D convolution, a batchnorm and a ReLU.
    They are executed in a sequential manner.
    ...
    Args:
        ...
        padding (int or tuple): padding of the convolution
    """
    def __init__(self, in_channels, out_channels, kernel_size, stride, isPadding=True, isBias=False,dilation=2):
        super(DilatedConv2dBatchReLU, self).__init__()
        ...
        if isPadding == True:
            self.layers = nn.Sequential(
                nn.Conv2d(self.in_channels, self.out_channels, self.kernel_size, self.stride, self.padding, bias=self.isBias,dilation=2),
                nn.BatchNorm2d(self.out_channels),
                nn.ReLU(inplace=True)
            )
        else:
            self.layers = nn.Sequential(
                nn.Conv2d(self.in_channels, self.out_channels, self.kernel_size, self.stride, 0, bias=self.isBias, dilation=2),
                nn.BatchNorm2d(self.out_channels),
                nn.ReLU(inplace=True)
            )
...
class hourGlass1(nn.Module):
    def __init__(self, in_channels):
        ...
        self.convDown = Conv2dBatchReLU(self.in_channels, self.in_channels, self.kernel_size, 2)
        self.convUp = nn.Upsample(scale_factor=2)
    ...
    def forward(self, x):
       # print(x.size())
        ...
        down2 = self.down2(down1) #32
        up2   = self.up1(down2) #64
        #print(down1.size(), down2.size(), up2.size())
        add1  = down1 + up2
        return x + self.up2(add1)
