...
class LPRNet2(nn.Module):
    def __init__(self, lpr_max_len, phase, class_num, dropout_rate):
        super(LPRNet2, self).__init__()
        self.phase = phase
        self.lpr_max_len = lpr_max_len
        self.class_num = class_num
        cfg = []
        ...
        self.container = nn.Sequential(
            nn.Conv2d(in_channels=416, out_channels=self.class_num, kernel_size=(1, 1), stride=(1, 1)),
            # nn.BatchNorm2d(num_features=self.class_num),
            # nn.ReLU(),
            # nn.Conv2d(in_channels=self.class_num, out_channels=self.lpr_max_len+1, kernel_size=3, stride=2),
            # nn.ReLU(),
        )
        self.cnn1=nn.Conv2d(in_channels=32, out_channels=32, kernel_size=4, stride=4)
        self.cnn2=nn.Conv2d(in_channels=128, out_channels=128, kernel_size=2, stride=2)
    def forward(self, x):
        keep_features = list()
        stage1 = self.layers[0](x)
        out1 = self.cnn1(stage1)
        #print(out1.size())
        stage2 = self.layers[1](stage1)
        out2 = self.cnn2(stage2)
        #print(out2.size())
        stage3=self.layers[2](stage2)
        #print(stage3.size())
        keep_features.append(out1)
        keep_features.append(out2)
        keep_features.append(stage3)

        # global_context = list()
        # for i, f in enumerate(keep_features):
        #     if i in [0]:
        #         f = nn.Conv2d(in_channels=32, out_channels=32, kernel_size=4, stride=4)(f)
        #     if i in [1]:
        #         f = nn.Conv2d(in_channels=128, out_channels=128, kernel_size=2, stride=2)(f)
        #     global_context.append(f)
        x = torch.cat(keep_features, 1)
        x = self.container(x)
        n,c,h,w=x.size()
        #print(x.size())
        x = x.view(n,c,-1)
        #print(x.size())
        logits = x.permute(2, 0, 1)
        #print(logits.size())
        return logits
