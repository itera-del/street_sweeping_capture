...
def gen_plates_double(N, path='imgs'):
    ...
    for i in range(N):
        ...
        if len(s[0])==1:
            ...
            org1 = 5
        ImageDraw.Draw(board).text((org1, -20), s[0], (0,0,0), font=font1)
        
        if len(s[1])==4:
            org2 = 130
        elif len(s[1])==5:
            org2 = 90
        ImageDraw.Draw(board).text((org2, 100), s[1], (0,0,0), font=font2)
...
if __name__ == '__main__':
    ...
    BOARD = cv2.imread('src_board/plate_double.jpg')
    gen_plates_double(3000, 'imgs_double')
