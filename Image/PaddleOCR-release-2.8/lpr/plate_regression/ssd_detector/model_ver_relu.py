...
class InvertedConcatResidual(nn.Module):
    def __init__(self, inp, oup, stride, expand_ratio):
        super(InvertedConcatResidual, self).__init__()
        self.stride = stride
        assert stride in [1, 2]
...
class InvertedResidualSe(nn.Module):
    def __init__(self, inp, oup, stride, expand_ratio):
        ...
        self.stride = stride
        self.oup = oup
        # self.se_layer = SELayer(oup, reduction=8)
        assert stride in [1, 2]
...
class InvertedResidualActivation(nn.Module):
    def __init__(self, inp, oup, stride, expand_ratio, activation_function='relu6'):
        super(InvertedResidualActivation, self).__init__()
        self.stride = stride
        assert stride in [1, 2]
        assert activation_function in ['relu6', 'hard_swish']
        hidden_dim = round(inp * expand_ratio)
        self.use_res_connect = self.stride == 1 and inp == oup
        self.activation_function = nn.ReLU6(inplace=True) if (activation_function == 'relu6') else HardSwish(
            inplace=True)
...
class MobileNetSE(nn.Module):
    def __init__(self, n_class=1000, width_mult=1., input_channel=32, last_channel=1280,
                 interverted_residual_setting=None):
        super(MobileNetSE, self).__init__()
...
class MobileNetSmallSe(nn.Module):
    def __init__(self, n_class=1000, width_mult=1., input_channel=32, last_channel=1280,
                 interverted_residual_setting=None):
        super(MobileNetSmallSe, self).__init__()
        block = InvertedResidualSe
        if interverted_residual_setting is None:
            interverted_residual_setting = [
                # t, c, n, s
...
class MobileNetV1SE(nn.Module):
    def __init__(self, n_class=1000, width_mult=1., input_channel=32, last_channel=1024,
                 interverted_residual_setting=None):
        super(MobileNetV1SE, self).__init__()
        block = InvertedDepthPointBlock
        if interverted_residual_setting is None:
            interverted_residual_setting = [
                # c, n, s
        ...
        for c, n, s in interverted_residual_setting:
            ...
            for i in range(n):
                if i == 0:
                    ...
                    self.features.append(block(input_channel, output_channel, 1))
                self.features.append(SELayer(output_channel, reduction=8))
                input_channel = output_channel
        # make it nn.Sequential
        if self.last_channel != input_channel:
            self.features.append(conv_1x1_bn(input_channel, self.last_channel))
        self.features.append(SELayer(self.last_channel, reduction=8))
        self.features = nn.Sequential(*self.features)
        # building classifier
        self.classifier = nn.Sequential(
            nn.Dropout(0.2),
...
class MobileNetConcat(nn.Module):
    def __init__(self, n_class=1000, width_mult=1., input_channel=32, last_channel=1280,
                 interverted_residual_setting=None):
        super(MobileNetConcat, self).__init__()
        block = InvertedConcatResidual
        if interverted_residual_setting is None:
            interverted_residual_setting = [
                # t, c, n, s
                [1, 16, 1, 1],
...
class MobileNetHardSwishV2(nn.Module):
    def __init__(self, n_class=1000, width_mult=1., input_channel=32, last_channel=1280,
                 interverted_residual_setting=None):
        super(MobileNetHardSwishV2, self).__init__()
        block = InvertedResidualActivation
        if interverted_residual_setting is None:
            interverted_residual_setting = [
                # t, c, n, s, a
                [1, 16, 1, 1, 'relu6'],
                ...
                [6, 64, 4, 2, 'hard_swish'],
                [6, 96, 3, 1, 'hard_swish'],
                [6, 160, 3, 2, 'hard_swish'],
                [6, 320, 1, 1, 'hard_swish'],
            ]
        ...
        self.features = [conv_bn(3, input_channel, 2)]
        # building inverted residual blocks
        for t, c, n, s, a in interverted_residual_setting:
            output_channel = int(c * width_mult)
            for i in range(n):
                if i == 0:
                    self.features.append(block(input_channel, output_channel, s, expand_ratio=t, activation_function=a))
                else:
                    self.features.append(block(input_channel, output_channel, 1, expand_ratio=t, activation_function=a))
                input_channel = output_channel
        # building last several layers
        self.features.append(conv_1x1_bn(input_channel, self.last_channel))
        # make it nn.Sequential
...
class MobileNetV3Real(nn.Module):
    def __init__(self, n_class=1000, width_mult=1., input_channel=16, last_channel=1280,
                 interverted_residual_setting=None):
        super(MobileNetV3Real, self).__init__()
        block = InvertedResidualV3
        if interverted_residual_setting is None:
            interverted_residual_setting = [
                # expand size, c, s, a, kernel size, use_se
