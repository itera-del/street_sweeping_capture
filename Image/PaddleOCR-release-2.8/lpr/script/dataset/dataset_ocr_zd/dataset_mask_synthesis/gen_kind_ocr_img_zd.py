...
from script.dataset.dataset_ocr_zd.dataset_mask.gen_ocr_img import kind_num_roi_to_bbox, get_resize_shape
...
def gen_kind_ocr_img(args):
    ...
    for dataset_idx in tqdm(range(len(args.input_dir_list))):
        ...
        for idx in tqdm(range(len(img_list))):
            ...
            xml_path = os.path.join(dataset_dir, args.input_xml_dir, img_name.replace(".jpg", ".xml"))
            tqdm.write(img_path)
            ...
            if 'kind' in seg_bbox and len(kind_id):
                if kind_id in args.find_kind_list:
                    ...
                    output_img_path = os.path.join(args.output_dir, args.output_img_format.format(kind_id), output_img_name)
                    create_folder(os.path.dirname(output_img_path))
...
if __name__ == '__main__':
    ...
    args.input_dir_list = [
        ...
        "/yuanhuan/data/image/LicensePlate_ocr/training/seg_zd/data_crop_0/",
    ]
    ...
    args.output_kind_format = "data_kind/{}"
    args.output_img_format = "data/{}"
