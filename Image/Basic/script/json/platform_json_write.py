...
class PlatformJsonWriter(object):
    ...
    def write_meta_json(
        self, meta_template_path, meta_json_path, task_name="task", data_size=300
    ):
        ...
        sub_meta = meta.copy()
        sub_meta["task"]["id"] = 0
        sub_meta["task"]["name"] = task_name
        sub_meta["task"]["size"] = data_size
        sub_meta["task"]["stop_frame"] = data_size - 1
        sub_meta["task"]["created"] = sub_meta["task"]["updated"] = sub_meta[
            "dumped"
        ] = get_time()
        ...
        with open(meta_json_path, "w") as f:
            f.write(
                json.dumps(
                    sub_meta, ensure_ascii=False, indent=4, separators=(",", ":")
                )
            )
    ...
    def write_json(
        ...
        with open(jsonpath, "w") as f:
            f.write(
                json.dumps(
                    json_file, ensure_ascii=False, indent=4, separators=(",", ":")
                )
            )
        return
    ...
    def add_det_tag(self, json_file, rect_list):
        for i in range(len(rect_list)):
            if len(rect_list[i]) == 5:
                ...
                new_shape["points"] = (
                    np.array([x1, y1, x2, y2], dtype=int).reshape(-1).tolist()
                )
