    ...
    for idx in tqdm(range(len(json_list))):
        ...
        for track in annotation['shapes']:
            ...
            if type == 'rectangle':
                ...
                x1 = max(int(points[0]), 0) 
                y1 = max(int(points[1]), 0) 
                ...
                x1 = max(int(min(points_list[:, 0])), 0)
                y1 = max(int(min(points_list[:, 1])), 0)
                x2 = min(int(max(points_list[:, 0])), img_width - 1)
                y2 = min(int(max(points_list[:, 1])), img_height - 1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_dir', type=str, default="/yuanhuan/mtyan/head_data/head_dataset_v2/head_dataset_v2/") 
