...
def xml_2_platform_json(args):
    ...
    for idx in tqdm(range(len(xml_list))):
        ...
        for object in root.findall('object'):
            ...
            elif classname == "motorcyclist":
                bndbox.extend(["motorcycle"])
            elif classname == "bicyclist":
                bndbox.extend(["bicycle"])
            elif classname == "license_plate" or "license_plate" in classname:      # "license_plate" & "license_plate_车牌号"
            ...
            if args.add_license_attribute:
                if classname == "license":
                    if args.with_color:
                        bndbox.extend([[{"name": "number", "value": ""}, {"name": "color", "value": "white"}, {"name": "column", "value": "single"}, {"name": "status", "value": "n"}]])
                    else:
                        bndbox.extend([[{"name": "number", "value": ""}, {"name": "column", "value": "single"}, {"name": "status", "value": "n"}]])
                elif "license" in classname:
                    if args.with_color:
                        license_plate_color = classname.split('license_plate_')[1].split('_')[1]
                        license_plate_ocr = classname.split('license_plate_')[1].split('_')[0]
                        license_plate_column = classname.split('license_plate_')[1].split('_')[2]
                        license_plate_column = "Single" if license_plate_column == "single" else "Double"
                        bndbox.extend([[{"name": "number", "value": license_plate_ocr}, {"name": "color", "value": license_plate_color}, {"name": "column", "value": license_plate_column}, {"name": "status", "value": "n"}]])
                    else:
                        license_plate_ocr = classname.split('license_plate_')[1]
                        bndbox.extend([[{"name": "number", "value": license_plate_ocr}, {"name": "column", "value": "single"}, {"name": "status", "value": "n"}]])
