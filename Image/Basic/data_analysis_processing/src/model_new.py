...
class MODEL_NEW():
    def __init__(self, path, result, is_dinov2=False, is_crop_image=False):
        '''
        ...
        # NOTE: path is str('folder_path') or [image_list]
        ...
        self.is_crop_image= is_crop_image
    ...
    def create_model(self, cc_threshold=0.8):
        ...
        if self.is_dinov2:
            ...
            self.fd.run(model_path="/defaultShare/data_closing/models/dinov2/dinov2_vitb14.onnx", cc_threshold=cc_threshold, d=786)
        else:
    ...
    def find_duplicate_images(self, num=20):
        ...
        for img_path in tqdm(list_of_duplicates):
            self.copy_duplicate_image(img_path)
        # with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
    ...
    def find_outlier_image(self, thresh=None, num=20):
        ...
        for img_path in tqdm(list_of_outliers):
            self.copy_outliers_image(img_path)
        # with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
    ...
    def remove_duplicate_images(self):
        ...
        print(f"Found {len(set(list_of_duplicates))} highly similar images to discard")
        duplicate_image_txt = os.path.join(self.result, 'duplicate_image.txt')
        with open(duplicate_image_txt, "w") as f:
            for idx in range(len(list_of_duplicates)):
                f.write(list_of_duplicates[idx])
                f.write("\n")
            
    def copy_broken_image(self, img_path):
        name = os.path.basename(img_path)
        if self.is_crop_image:
            img_path = [raw_path for raw_path in self.path if os.path.basename(raw_path) == name][0]
        shutil.copy(img_path, os.path.join(self.result, 'results', 'broken_images', name))

    def copy_duplicate_image(self, img_path):
        name = os.path.basename(img_path)
        if self.is_crop_image:
            img_path = os.path.join(self.args.images_path, name)
