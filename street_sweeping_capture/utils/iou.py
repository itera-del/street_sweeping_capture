...
def match_object_list_license_plate(object_roi_list, roi_list):
    # sort_key
    def sort_key(data):
        return data[-1]
    ...
    for idx in range(len(object_roi_list)):
        match_roi_idx = object_roi_list[idx][0:4]

        # 方案二：计算车牌框完全位于车框内
        bool_in = bool_box_in_roi(roi_list, match_roi_idx)
        if bool_in:
            # 默认车牌均是在车辆的下沿
            matched_roi_list.append(object_roi_list[idx])
