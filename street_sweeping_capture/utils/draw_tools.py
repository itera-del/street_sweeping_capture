...
color_dict = {
                ...
                "side_face": (255, 255, 255),
                "face_occlusion": (255, 255, 255),
                "person": (255, 255, 255),
                "bicyclist": (255, 255, 255),
                "motorcyclist": (255, 255, 255),
            }
...
class DrawApi():
    ...
    def draw_bbox_info(self, img, bbox_info, capture_container=None, capture_res_container=None, mode='xywh'):
        ...
        for _, capture_res_idy in capture_res_container.items():
            ...
            if capture_res_idy['capture']['capture_frame_num'] < draw_alarm_frame_num_threshold:
                capture_bool = True
            # capture_info = "Shooting: " if capture_bool else "Shot: "
            # capture_info = "抓拍结果: "
            capture_info = ""

            if self.demo_type == "lpr":
                
                # if len(capture_res_idy['plate_info']['num']) > 1 and capture_res_idy['plate_info']['num'][:2] == "粤A":
                #     continue

                if self.country_type == "zd":
                    text = "{}{}_{}_{}_{}_{}_{}_{}".format( capture_info, capture_res_idy['plate_info']['kind'], capture_res_idy['plate_info']['num'], capture_res_idy['plate_info']['country'], capture_res_idy['plate_info']['city'], capture_res_idy['plate_info']['car_type'], capture_res_idy['plate_info']['color'], capture_res_idy['plate_info']['column'] )
                elif self.country_type == "morocco":
                    text = "{}{}_{}_{}".format( capture_info, capture_res_idy['plate_info']['kind'], capture_res_idy['plate_info']['city'], capture_res_idy['plate_info']['num'] )
                elif self.country_type == "brazil":
                    text = "{}{}_{}".format( capture_info, capture_res_idy['plate_info']['num'], capture_res_idy['plate_info']['column'])
                else:
                    # text = "{}: {}_{}_{}".format( capture_info, capture_res_idy['plate_info']['num'], capture_res_idy['plate_info']['column'], capture_res_idy['plate_info']['color'] )
                    # text = "{}{}_{}".format( capture_info, capture_res_idy['plate_info']['num'], capture_res_idy['plate_info']['color'] )
                    text = "{}_{}_{}".format( capture_res_idy['plate_info']['num'], capture_res_idy['plate_info']['color'], capture_res_idy['car_info']['attri'] )
                
                text_size = cv2.getTextSize(text, 0, 3, 2)
                cv2.rectangle(img, (x, y), (x + int(text_size[0][0] * 0.88), y + int(text_size[0][1] * 1.6)), (255,255,255), -1)
                # img = NiceBox(img, (x, y, x + int(text_size[0][0] * 1.08), y + int(text_size[0][1] * 1.6)), (255,255,255), thickness=5)
                
                if not capture_bool:
                    # img = cv2.putText(img, text, (x, y + text_size[0][1]), cv2.FONT_HERSHEY_COMPLEX, 3, color_dict["plate"], 2)
                    img = cv2.putText(img, text, (x, y + text_size[0][1]), cv2.FONT_HERSHEY_COMPLEX, 2, color_dict["plate"], 1)
                    ...
                    img = cv2.putText(img, text, (x, y + text_size[0][1]), cv2.FONT_HERSHEY_COMPLEX, 2, color_dict["plate_capture"], 1)
                ...
                bbox_loc = [bbox_info_crop['plate_info']['roi'] for bbox_info_crop in bbox_info_crop if bbox_info_crop['track_id'] == track_id][0]
                ...
                bbox_crop = cv2.resize(bbox_crop, (225, 150)) if capture_res_idy['plate_info']['column'] == "Double" else cv2.resize(bbox_crop, (375, 150))
                # img[y - 25: y + bbox_crop.shape[0] - 25, x + int(text_size[0][0] * 1.1) : x + int(text_size[0][0] * 1.1) + bbox_crop.shape[1]] = bbox_crop
        ...
        for idx in range(len(bbox_info)):
            ...
            #     if len(bbox_info_idx['plate_info']['num_stabel']) > 1 and bbox_info_idx['plate_info']['num_stabel'][:2] == "粤A":
            #         capture_bool = False
            ...
            if self.demo_type == "lpr":
                ...
                if not capture_bool and len(bbox_info_idx['car_info']['roi']):
                    ...
                    img = cv_plot_rectangle(img, bbox_info_idx['car_info']['roi'], mode=mode, color=color_dict[bbox_info_idx['car_info']['attri']])
                    # img = NiceBox(img, bbox_info_idx['car_info']['roi'], color_dict["car"], thickness=3, mask=False)

                    img = cv2.putText(img, "{}".format( bbox_info_idx['car_info']['attri'] ), (bbox_info_idx['car_info']['roi'][0], bbox_info_idx['car_info']['roi'][1] - 10), 
                                        cv2.FONT_HERSHEY_COMPLEX, 1, color_dict[bbox_info_idx['car_info']['attri']], 2)
                ...
                if len(bbox_info_idx['plate_info']['roi']):
                    ...
                    if self.country_type == "zd":
                        ...
                        text = "{}#{}_{:.2f}".format( bbox_info_idx['plate_info']['kind'], bbox_info_idx['plate_info']['num'], bbox_info_idx['plate_info']['score'])
                    elif self.country_type == "morocco":
                        text = "{}{}{}_{:.2f}".format( bbox_info_idx['plate_info']['kind'], bbox_info_idx['plate_info']['city'], bbox_info_idx['plate_info']['num'], bbox_info_idx['plate_info']['score'])
                    elif self.country_type == "brazil":
                        text = "{}{}".format( bbox_info_idx['plate_info']['kind'], bbox_info_idx['plate_info']['num'])
                    else:                        
                        ...
                        # text = "{}_{}{}_{}_{}_{}_{}".format( bbox_info_idx['track_id'], bbox_info_idx['plate_info']['kind'], bbox_info_idx['plate_info']['num'], bbox_info_idx['plate_info']['color'], int(bbox_info_idx['plate_info']['dist_left_lane_line']), int(bbox_info_idx['plate_info']['dist_right_lane_line']), bbox_info_idx['state']['obj_num'])
                        # text = "{}{}".format( bbox_info_idx['plate_info']['kind'], bbox_info_idx['plate_info']['num'])
                        text = "{}{}".format( bbox_info_idx['plate_info']['kind_stabel'], bbox_info_idx['plate_info']['num_stabel'])
                    ...
                    if not capture_bool:
                        ...
                        # img = cv2.putText(img, text, (bbox_info_idx['plate_info']['roi'][0], bbox_info_idx['plate_info']['roi'][1] - 15), 
                        #                     cv2.FONT_HERSHEY_COMPLEX, 3, color_dict["plate"], 2)
                        img = cv2.putText(img, text, (bbox_info_idx['plate_info']['roi'][0], bbox_info_idx['plate_info']['roi'][1] - 15), 
                                            cv2.FONT_HERSHEY_COMPLEX, 2, color_dict["plate"], 1)
                        ...
                        img = cv2.putText(img, text, (bbox_info_idx['plate_info']['roi'][0], bbox_info_idx['plate_info']['roi'][1] - 15), 
                                            cv2.FONT_HERSHEY_COMPLEX, 2, color_dict["plate_capture"], 1)
                    ...
                    if len(bbox_info_idx['plate_info']['city_roi']):
                        img = cv_plot_rectangle(img, bbox_info_idx['plate_info']['city_roi'], mode=mode, color=color_dict["plate"], thickness=3)

            elif self.demo_type == "face":

                # person
                if len(bbox_info_idx['person_info']['roi']):
                    bbox_info_idx['person_info']['roi'] = [int(b + 0.5) for b in bbox_info_idx['person_info']['roi'][:4]]
                    if not capture_bool and len(bbox_info_idx['person_info']['roi']):
                        img = cv_plot_rectangle(img, bbox_info_idx['person_info']['roi'], mode=mode, color=color_dict["car"])
                        # img = NiceBox(img, bbox_info_idx['person_info']['roi'], color_dict["car"], thickness=3, mask=False)

                        img = cv2.putText(img, "{}_{}".format( bbox_info_idx['track_id'], bbox_info_idx['state']['frame_num']), (bbox_info_idx['person_info']['roi'][0], bbox_info_idx['person_info']['roi'][1] - 10), 
                                            cv2.FONT_HERSHEY_COMPLEX, 1, color_dict["car"], 2)

                        # img = cv2.putText(img, "{}_{}_{}_{}_{:.2f}_{:.2f}".format( bbox_info_idx['state']['up_down_state'], bbox_info_idx['state']['up_down_state_frame_num'], bbox_info_idx['state']['left_right_state'], bbox_info_idx['state']['left_right_state_frame_num'], bbox_info_idx['state']['up_down_speed'], bbox_info_idx['state']['left_right_speed'] ), (bbox_info_idx['person_info']['roi'][0], bbox_info_idx['person_info']['roi'][3] - 10), 
                        #                     cv2.FONT_HERSHEY_COMPLEX, 1, color_dict["car"], 2)
                    elif len(bbox_info_idx['person_info']['roi']):
                        img = cv_plot_rectangle(img, bbox_info_idx['person_info']['roi'], mode=mode, color=color_dict["car_capture"])
                        # img = NiceBox(img, bbox_info_idx['person_info']['roi'], color_dict["car_capture"], thickness=3, mask=False)
                ...
                # face
                if len(bbox_info_idx['face_info']['roi']):
    ...
    def draw_bbox_detect(self, img, bboxes, mode='ltrb'):

        for key in bboxes.keys():
            for idy in range(len(bboxes[key])):
                ...
                img = cv2.putText(img, "{}".format(key), (int(loc[0]), int(loc[1])), cv2.FONT_HERSHEY_COMPLEX, 1, color, 2)
