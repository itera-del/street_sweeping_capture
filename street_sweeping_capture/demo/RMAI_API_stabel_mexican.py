...
from Image.detection2d.rm_ai_mmdet_3_3_yolox.my_tools.yolox_detector import YOLOXDetector
...
# from Image.Demo.street_sweeping_capture.model.LPR_detect import LPRDetectCaffe, LPRDetectOpenVINO, LPRDetectCaffe_2_0, LPRDetectCaffe_3_0
...
# from Image.recognition2d.lpr.infer.lpr_seg_ocr import LPRSegOcrcaffe, LPRSegOcrMoroccocaffe
...
# from Image.Demo.street_sweeping_capture.info.options_lpr_us import options
# from Image.Demo.street_sweeping_capture.info.options_lpr_eu import options
# from Image.Demo.street_sweeping_capture.info.options_lpr_chile import options
...
# from Image.Demo.street_sweeping_capture.info.options_lpr_morocco_police import options
# from Image.Demo.street_sweeping_capture.info.options_face import options
# from Image.Demo.street_sweeping_capture.info.options_face_person import options
# from Image.Demo.street_sweeping_capture.info.options_lpr_huanwei import options
from Image.Demo.street_sweeping_capture.info.options_lpr_mexican import options
...
# 墨西哥 ONNX 识别走 lpr_mexican.LPROnnx（与 Paddle 对齐的预处理与 (T,C) 解码），勿改回 lpr 以免旧 transpose 超长乱码
import Image.recognition2d.script.paddle.infer.lpr_mexican as paddle_lpr
...
class CaptureApi():
    ...
    def param_init(self):
        ...
        # mexican：OCR 后保存车牌裁剪 ROI，文件名尾缀为识别结果（排查完将 lpr_ocr_debug_save_bool 置 False）
        self._run_frame_idx = -1
        if getattr(self, "country_type", None) == "mexican":
            self.lpr_ocr_debug_save_bool = True
            self.lpr_ocr_debug_save_dir = "/yuanhuan/data/image/RM_MEXICAN/original/Argentina/Argentina/Argentina_Mexican_20260331/test/test_20260420/debug"
            self._lpr_ocr_debug_seq = 0
        else:
            self.lpr_ocr_debug_save_bool = False

    def _sanitize_ocr_for_filename(self, ocr_text, max_len=80):
        if ocr_text is None:
            ocr_text = ""
        s = str(ocr_text).strip()
        if not s:
            return "empty"
        for c in r'\/:*?"<>|\n\t\r':
            s = s.replace(c, "_")
    ...
    def _save_lpr_ocr_debug_crop_if_needed(self, crop_img, bbox_info_idx, x1, y1, x2, y2, ocr_text=""):
        if not getattr(self, "lpr_ocr_debug_save_bool", False):
            return
        if crop_img is None or crop_img.size == 0:
            return
        self._lpr_ocr_debug_seq += 1
        os.makedirs(self.lpr_ocr_debug_save_dir, exist_ok=True)
        tid = int(bbox_info_idx.get("track_id", -1))
        w, h = int(crop_img.shape[1]), int(crop_img.shape[0])
        ocr_safe = self._sanitize_ocr_for_filename(ocr_text)
        fn = "f{:06d}_tid{}_n{:05d}_wh{}x{}_xyxy_{}_{}_{}_{}__ocr_{}.jpg".format(
            int(self._run_frame_idx), tid, self._lpr_ocr_debug_seq, w, h, x1, y1, x2, y2, ocr_safe
        )
        cv2.imwrite(os.path.join(self.lpr_ocr_debug_save_dir, fn), crop_img)
    ...
    def model_init(self):
        ...
        if self.demo_type == "lpr":
            # detector
            if options.ssd_bool:
                if self.country_type == "china" or self.country_type == "brazil" or self.country_type == "us" or self.country_type == "eu" or self.country_type == "chile" or self.country_type == "huanwei" or self.country_type == "mexican":
                    ...
                    self.detector = SSDDetector(prototxt=options.ssd_prototxt, model_path=options.ssd_model_path, ssd_caffe_bool=options.ssd_caffe_bool, ssd_openvino_bool=options.ssd_openvino_bool, merge_class_bool=options.object_attri_merge_bool, gpu_bool=True)
                elif self.country_type == "zd" or self.country_type == "morocco":
                    # self.detector = LPRDetectCaffe(options.ssd_prototxt, options.ssd_model_path, class_name=options.ssd_class_name, gpu_bool=options.gpu_bool, conf_thres=options.ssd_conf_thres)
                    # self.detector = LPRDetectCaffe_2_0(options.ssd_prototxt, options.ssd_model_path, class_name=options.ssd_class_name, gpu_bool=options.gpu_bool, conf_thres=options.ssd_conf_thres)
                    self.detector = LPRDetectCaffe_3_0(options.ssd_prototxt, options.ssd_model_path, class_name=options.ssd_class_name, gpu_bool=options.gpu_bool, conf_thres=options.ssd_conf_thres)
            ...
            elif options.yolox_bool:
                self.detector = YOLOXDetector(options.yolox_config, options.yolox_checkpoint, class_name=options.yolox_class_name, threshold_list=options.yolox_threshold_list, device='cuda:0')
            ...
            elif self.country_type == "us":
                if options.lpr_caffe_bool:
                    if options.lpr_paddle_bool: 
                        self.lpr = None
                    else:
                        self.lpr = LPRCaffe(options.us.ocr_caffe_prototxt, options.us.ocr_caffe_model_path, input_shape=options.us.input_shape, ocr_labels=options.us.ocr_labels, padding_bool=options.us.padding_bool, prefix_beam_search_bool=options.us.ocr_prefix_beam_search_bool, gpu_bool=options.gpu_bool)

                elif options.lpr_pytorch_bool:       
                    if options.lpr_paddle_bool: 
                        self.lpr = paddle_lpr.LPRCaffe(options.us.ocr_caffe_model_path, options.us.ocr_caffe_prototxt, options.us.ocr_labels_dict_path, img_shape=options.us.input_shape, padding_bool=options.us.padding_bool, gpu_bool=options.gpu_bool)
                    else:
                        self.lpr = LPRCaffe(options.us.ocr_caffe_prototxt, options.us.ocr_caffe_model_path, input_shape=options.us.input_shape, ocr_labels=options.us.ocr_labels, padding_bool=options.us.padding_bool, prefix_beam_search_bool=options.us.ocr_prefix_beam_search_bool, gpu_bool=options.gpu_bool)

                elif options.lpr_onnx_bool:
                    if options.lpr_paddle_bool: 
                        self.lpr = paddle_lpr.LPROnnx(options.us.ocr_onnx_model_path, options.us.ocr_labels_dict_path, img_shape=options.us.input_shape, padding_bool=options.us.padding_bool, gpu_bool=options.gpu_bool)
                    else:
                        self.lpr = None

            elif self.country_type == "eu":
                if options.lpr_caffe_bool:
                    if options.lpr_paddle_bool: 
                        self.lpr = paddle_lpr.LPRCaffe(options.eu.ocr_caffe_model_path, options.eu.ocr_caffe_prototxt, options.eu.ocr_labels_dict_path, img_shape=options.eu.input_shape, padding_bool=options.eu.padding_bool, gpu_bool=options.gpu_bool)
                    else:
                        self.lpr = LPRCaffe(options.eu.ocr_caffe_prototxt, options.eu.ocr_caffe_model_path, input_shape=options.eu.input_shape, ocr_labels=options.eu.ocr_labels, padding_bool=options.eu.padding_bool, prefix_beam_search_bool=options.eu.ocr_prefix_beam_search_bool, gpu_bool=options.gpu_bool)
                    ...
                    if options.lpr_paddle_bool: 
                        self.lpr = paddle_lpr.LPROnnx(options.eu.ocr_onnx_model_path, options.eu.ocr_labels_dict_path, img_shape=options.eu.input_shape, padding_bool=options.eu.padding_bool, gpu_bool=options.gpu_bool)
                    else:
                        self.lpr = None

            elif self.country_type == "chile":
                if options.lpr_caffe_bool:
                    if options.lpr_paddle_bool: 
                        self.lpr = None
                    else:
                        self.lpr = LPRCaffe(options.chile.ocr_caffe_prototxt, options.chile.ocr_caffe_model_path, input_shape=options.chile.input_shape, ocr_labels=options.chile.ocr_labels, padding_bool=options.chile.padding_bool, prefix_beam_search_bool=options.chile.ocr_prefix_beam_search_bool, gpu_bool=options.gpu_bool)

                elif options.lpr_pytorch_bool:       
                    if options.lpr_paddle_bool: 
                        self.lpr = paddle_lpr.LPRCaffe(options.chile.ocr_caffe_model_path, options.chile.ocr_caffe_prototxt, options.chile.ocr_labels_dict_path, img_shape=options.chile.input_shape, padding_bool=options.chile.padding_bool, gpu_bool=options.gpu_bool)
                    else:
                        self.lpr = LPRCaffe(options.chile.ocr_caffe_prototxt, options.chile.ocr_caffe_model_path, input_shape=options.chile.input_shape, ocr_labels=options.chile.ocr_labels, padding_bool=options.chile.padding_bool, prefix_beam_search_bool=options.chile.ocr_prefix_beam_search_bool, gpu_bool=options.gpu_bool)

                elif options.lpr_onnx_bool:
                    if options.lpr_paddle_bool: 
                        self.lpr = paddle_lpr.LPROnnx(options.chile.ocr_onnx_model_path, options.chile.ocr_labels_dict_path, img_shape=options.chile.input_shape, padding_bool=options.chile.padding_bool, gpu_bool=options.gpu_bool)
                    else:
                        self.lpr = None

            elif self.country_type == "zd":
                if options.lpr_caffe_bool:
                    self.lpr_seg_ocr = LPRSegOcrcaffe(options.zd.seg_caffe_prototxt, options.zd.seg_caffe_model_path, options.zd.ocr_caffe_prototxt, options.zd.ocr_caffe_model_path, gpu_bool=options.gpu_bool, dict_path=options.zd.dict_path)
            ...
            elif self.country_type == "morocco":
                if options.lpr_caffe_bool:
                    self.lpr_seg_ocr = LPRSegOcrMoroccocaffe(options.morocco.seg_caffe_prototxt, options.morocco.seg_caffe_model_path, options.morocco.ocr_caffe_prototxt, options.morocco.ocr_caffe_model_path, gpu_bool=options.gpu_bool, dict_path=options.morocco.dict_path)
            ...
            elif self.country_type == "huanwei":
                if options.lpr_caffe_bool:
                    if options.lpr_paddle_bool: 
                        self.lpr = paddle_lpr.LPRCaffe(options.huanwei.ocr_caffe_model_path, options.huanwei.ocr_caffe_prototxt, options.huanwei.ocr_labels_dict_path, img_shape=options.huanwei.input_shape, padding_bool=options.huanwei.padding_bool, gpu_bool=options.gpu_bool)
                    else:
                        self.lpr = LPRCaffe(options.huanwei.ocr_caffe_prototxt, options.huanwei.ocr_caffe_model_path, input_shape=options.huanwei.input_shape, ocr_labels=options.huanwei.ocr_labels, padding_bool=options.huanwei.padding_bool, prefix_beam_search_bool=options.huanwei.ocr_prefix_beam_search_bool, gpu_bool=options.gpu_bool)

                elif options.lpr_pytorch_bool:
                    if options.lpr_paddle_bool: 
                        self.lpr = None
                    else:
                        self.lpr = LPRPytorch(options.huanwei.ocr_pth_path, input_shape=options.huanwei.input_shape, ocr_labels=options.huanwei.ocr_labels, prefix_beam_search_bool=options.huanwei.ocr_prefix_beam_search_bool)

                elif options.lpr_onnx_bool:
                    if options.lpr_paddle_bool: 
                        self.lpr = paddle_lpr.LPROnnx(options.huanwei.ocr_onnx_model_path, options.huanwei.ocr_labels_dict_path, img_shape=options.huanwei.input_shape, padding_bool=options.huanwei.padding_bool, gpu_bool=options.gpu_bool)
                    else:
                        self.lpr = None
            
            elif self.country_type == "mexican":
                if options.lpr_caffe_bool:
                    if options.lpr_paddle_bool:
                        self.lpr = paddle_lpr.LPRCaffe(options.mexican.ocr_caffe_model_path, options.mexican.ocr_caffe_prototxt, options.mexican.ocr_labels_dict_path, img_shape=options.mexican.input_shape, padding_bool=options.mexican.padding_bool, gpu_bool=options.gpu_bool)
                    else:
                        self.lpr = None
                    ...
                    if options.lpr_paddle_bool:
                        self.lpr = paddle_lpr.LPROnnx(options.mexican.ocr_onnx_model_path, options.mexican.ocr_labels_dict_path, img_shape=options.mexican.input_shape, padding_bool=options.mexican.padding_bool, gpu_bool=options.gpu_bool)
                    else:
                        self.lpr = None
    ...
    def run(self, img, frame_idx):
        ...
        self._run_frame_idx = int(frame_idx)
        ...
        return bboxes, tracker_bboxes, bbox_info_list, bbox_state_container, capture_line_points, capture_container, capture_res_container
    

    def update_tracker_bboxes(self, bboxes):

        if self.demo_type == "lpr" and options.sort_type == "car" or (self.demo_type == "face" and options.sort_type == "person"):
            if options.object_attri_merge_bool:
                # tracker
                if options.object_attri_merge_name in bboxes:
                    dets = np.array(bboxes[options.object_attri_merge_name])
                else:
                    dets = np.empty((0, 5))
                ...
                # tracker
                dets = np.empty((0, 5))
                for idx in range(len(options.object_attri_name_list)):
                    car_attri_name_idx = options.object_attri_name_list[idx]
                    if car_attri_name_idx in bboxes:
                        dets = np.concatenate((dets, np.array(bboxes[car_attri_name_idx])), axis=0)
                        
                tracker_bboxes = self.mot_tracker.update(dets)

        elif (self.demo_type == "lpr" and options.sort_type == "plate" or (self.demo_type == "face" and options.sort_type == "face")):
            # tracker
            dets = np.empty((0, 5))
    ...
    def update_bbox_info(self, img, bboxes, tracker_bboxes):
        ...
        for idx in range(len(tracker_bboxes)):
            ...
            if self.demo_type == "lpr" and options.sort_type == "car":
                ...
                # 两阶段模型，不能使用跟踪框，应该使用检测框
                plate_bbox_list = []
                for idx in range(len(options.sort_class_name)):
                    sort_class_name = options.sort_class_name[idx]
                    if sort_class_name in bboxes:
                        for idy in range(len(bboxes[sort_class_name])):
                            plate_bbox_list.append([*bboxes[sort_class_name][idy]])
                # 求交集最大的车辆框
                match_plate_roi = match_bbox_iou(tracker_bbox[0:4], plate_bbox_list)
                if len(match_plate_roi):
                    bbox_info_dict['plate_info']['roi'] = np.array(match_plate_roi[0][:-1])
                else:
                    bbox_info_dict['plate_info']['roi'] = tracker_bbox[0:4]
                bbox_info_list.append(bbox_info_dict)

            elif self.demo_type == "face" and options.sort_type == "face":
                bbox_info_dict['track_id'] = tracker_bboxes[idx][-1]
                bbox_info_dict['face_info']['roi'] = tracker_bboxes[idx][0:4]
                bbox_info_list.append(bbox_info_dict)

            elif self.demo_type == "face" and options.sort_type == "person":
                tracker_bbox = tracker_bboxes[idx]
                bbox_info_dict['track_id'] = tracker_bbox[-1]
                bbox_info_dict['person_info']['roi'] = tracker_bbox[0:4]
                bbox_info_list.append(bbox_info_dict)
        ...
        for idx in range(len(bbox_info_list)):
            ...
            if self.demo_type == "lpr" and options.sort_type == "car":
                ...
                if options.object_attri_merge_bool:
                    ...
                    # 求交集最大的车辆框
                    match_object_roi = match_bbox_iou(bbox_info_idx['car_info']['roi'], car_bbox_list)
                    if len(match_object_roi):
                        bbox_info_idx['car_info']['attri'] = match_object_roi[0][-1]
    ...
    def update_plate_info( self, img, bboxes, bbox_info_list ):
        ...
        # sort plate info
        def sort_key(data):
            return data['plate_info']['roi'][3] - data['plate_info']['roi'][1] if len(data['plate_info']['roi']) else 0
        bbox_info_list.sort(key=sort_key, reverse=True)
        ...
        for idx in range(len(bbox_info_list)):
            ...
            if self.demo_type == "lpr":
                ...
                # # 按照车牌宽高过滤车牌
                # lpr_width =  bbox_info_idx['plate_info']['roi'][2] - bbox_info_idx['plate_info']['roi'][0]
                # lpr_height =  bbox_info_idx['plate_info']['roi'][3] - bbox_info_idx['plate_info']['roi'][1]
                # if not ((lpr_height > options.plate_height[0]) and \
                #     (lpr_height < options.plate_height[1]) and \
                #     (lpr_width > options.plate_width[0]) and \
                #     (lpr_width < options.plate_width[1])):
                #     continue
                        
                # print(bbox_info_idx['plate_info']['roi'][3] - bbox_info_idx['plate_info']['roi'][1])
                if self.demo_type == "lpr" and options.sort_type == "plate":
    
                    # car
                    if options.object_attri_merge_bool:
                        if options.object_attri_merge_name in bboxes:
                            object_roi_list = bboxes[options.object_attri_merge_name]
                            # 求相交同时置信度最高的车牌框
                            match_object_roi = match_object_list_license_plate(object_roi_list, bbox_info_idx['plate_info']['roi'])

                            if len(match_object_roi):
                                object_roi = match_object_roi[0][0:4]

                                bbox_info_idx['car_info']['roi'] = object_roi
                    else:
                        for car_attri_idx in range(len(options.object_attri_name_list)):
                            car_attri_name = options.object_attri_name_list[car_attri_idx]
                            if car_attri_name in bboxes:
                                object_roi_list = bboxes[car_attri_name]
                                # 求相交同时置信度最高的车牌框
                                match_object_roi = match_object_list_license_plate(object_roi_list, bbox_info_idx['plate_info']['roi'])
                ...
                if self.country_type == "china":
                    ...
                    if options.strategy.only_one_plate and plate_ocr_num >= 1:
                        break
                ...
                elif self.country_type == "us" or self.country_type == "eu" or self.country_type == "chile" or self.country_type == "mexican":
                    ...
                    if self.country_type == "mexican":
                        self._save_lpr_ocr_debug_crop_if_needed(
                            crop_img, bbox_info_idx, x1, y1, x2, y2, ocr_text=ocr_num
                        )
                    ...
                    seg_bbox, seg_info, ocr, ocr_score, ocr_ignore, _ = self.lpr_seg_ocr.run(crop_img)
                    ...
                    bbox_info_idx['plate_info']['column'] = 'Single' if crop_img_aspect > options.lpr_ocr_column_threshold else 'Double'
                    # bbox_info_idx['plate_info']['score'] = np.array(ocr_score).mean() if len(ocr_score) else 0.0
                    bbox_info_idx['plate_info']['score'] = np.array(ocr_score).min() if len(ocr_score) else 0.0
                    ...
                    if 'city' in seg_bbox:
                        bbox_info_idx['plate_info']['city_roi'] = [0, 0, 0, 0]
                        bbox_info_idx['plate_info']['city_roi'][0] = x1 + seg_bbox['city'][0][0]
                        bbox_info_idx['plate_info']['city_roi'][1] = y1 + seg_bbox['city'][0][1]
                        bbox_info_idx['plate_info']['city_roi'][2] = x1 + seg_bbox['city'][0][0] + seg_bbox['city'][0][2]
                        bbox_info_idx['plate_info']['city_roi'][3] = y1 + seg_bbox['city'][0][1] + seg_bbox['city'][0][3]
                    ...
                    bbox_info_idx['plate_info']['city'] = ""
                    bbox_info_idx['plate_info']['kind'] = ""
                    bbox_info_idx['plate_info']['num'] = ""

                    # if seg_info['kind'] == 'kind' and seg_info['city'] == 'city' and seg_info['num'] == 'num':
                    #     kind, city, num = split_plate_ocr(ocr)
                    #     if seg_info['kind'] == 'kind' and kind:
                    #         bbox_info_idx['plate_info']['kind'] = kind
                    #     if seg_info['city'] == 'city' and city:
                    #         bbox_info_idx['plate_info']['city'] = city
                    #     if seg_info['num'] == 'num' and num:
                    #         bbox_info_idx['plate_info']['num'] = num

                    # elif seg_info['city'] == 'city' and seg_info['num'] == 'num':
                    #     kind, city, num = split_plate_ocr_city(ocr)
                    #     if seg_info['city'] == 'city' and city:
                    #         bbox_info_idx['plate_info']['city'] = city
                    #     if seg_info['num'] == 'num' and num:
                    #         bbox_info_idx['plate_info']['num'] = num

                    # elif seg_info['num'] == 'num':
                    #     if seg_info['num'] == 'num' and ocr:
                    #         bbox_info_idx['plate_info']['num'] = ocr

                    kind, city, num = split_plate_ocr(ocr)
                    if kind == "" and  city == "" and num == "":
                        kind, city, num = split_plate_ocr_city(ocr)
                        if kind == "" and  city == "" and num == "":
                            num = ocr
                    bbox_info_idx['plate_info']['kind'] = kind
    ...
    def update_face_info( self, img, bboxes, bbox_info_list ):
        ...
        for idx in range(len(bbox_info_list)):
            ...
            if self.demo_type == "face" and options.sort_type == "face":

                if options.filter_face_by_person_bool == True:
                    ...
                    if options.object_attri_merge_bool:
                        if options.object_attri_merge_name in bboxes:
                            object_roi_list = bboxes[options.object_attri_merge_name]
                            # 求相交同时置信度最高的车牌框
                            match_object_roi = match_object_list_license_plate(object_roi_list, bbox_info_idx['face_info']['roi'])

                            if len(match_object_roi):
                                object_roi = match_object_roi[0][0:4]

                                bbox_info_idx['person_info']['roi'] = object_roi
                    ...
                    # filter
                    if len(bbox_info_idx['person_info']['roi']) == 0:
                        continue
                ...
                # 求交集最大的框
                match_object_roi = match_bbox_iou(bbox_info_idx['face_info']['roi'], face_bbox_list)
                if len(match_object_roi):
                    bbox_info_idx['face_info']['roi'] = np.array(match_object_roi[0][0:4])
                    if options.landmark_bool:
                        bbox_info_idx['face_info']['landmark'] = np.array(match_object_roi[0][4:14])
                        bbox_info_idx['face_info']['landmark_degree'] = landmark2degree(copy.deepcopy(bbox_info_idx['face_info']['landmark']))
                    elif options.landmark_degree_bool:
                        bbox_info_idx['face_info']['landmark_degree'] = np.array(match_object_roi[0][5])
                    elif options.landmark_degree_cls_bool:
                        bbox_info_idx['face_info']['landmark_positive_cls'] = "1" if match_object_roi[0][5] > 0.5 else "0"
                    else:
                        bbox_info_idx['face_info']['landmark_positive_cls'] = "1" 
            
            elif self.demo_type == "face" and options.sort_type == "person":

                # face
                if options.face_name in bboxes:
                    face_roi_list = bboxes[options.face_name]
                    # 求相交同时置信度最高的车牌框
                    match_object_roi = match_car_license_plate(bbox_info_idx['person_info']['roi'], face_roi_list)

                    if len(match_object_roi):
                        object_roi = match_object_roi[0][0:4]

                        bbox_info_idx['face_info']['roi'] = object_roi
                        bbox_info_idx['face_info']['landmark_positive_cls'] = "1" 
            ...
            for key, bbox_state_idy in self.params_dict['bbox_state_container'].items():
                ...
                if bbox_info_idx['track_id'] == bbox_state_idy['track_id']:
                    ...
                    if self.demo_type == "lpr":
                        ...
                        bbox_state_idy['person_info']['roi'] = bbox_info_idx['person_info']['roi'] 
                        bbox_state_idy['face_info']['roi'] = bbox_info_idx['face_info']['roi'] 
                        if options.sort_type == "face":
                        ...
                        elif options.sort_type == "person":
                            new_stable_loc = options.update_state_stable_loc_alpha * bbox_state_idy['state']['stable_loc'] +  (1 - options.update_state_stable_loc_alpha) * bbox_info_idx['person_info']['roi']
                    ...
                    if self.demo_type == "lpr":
                        ...
                        if bool_roi_lpr:
                            ...
                            if self.country_type == "china" or self.country_type == "us"  or self.country_type == "eu" or self.country_type == "chile" or self.country_type == "huanwei" or self.country_type == "mexican":
                        ...
                        if bool_add_lpr:
                            ...
                            bbox_state_idy['state']['car_attri_list'].append(bbox_info_idx['car_info']['attri'])
                            ...
                            if self.country_type == "morocco":
                                if len( bbox_state_idy['state']['lpr_city_list'] ) > options.lpr_ocr_state_container_length:
                            ...
                            if len( bbox_state_idy['state']['car_attri_list'] ) > options.lpr_city_state_container_length:
                                bbox_state_idy['state']['car_attri_list'].pop(0)
                        ...
                        #     bool_add_face = True
                        bool_add_face = False
                        if len(bbox_info_idx['face_info']['roi']) != 0 and \
                            (bbox_state_idy['face_info']['roi'][2] - bbox_state_idy['face_info']['roi'][0] > options.face_size_threshold[0] or \
                            bbox_state_idy['face_info']['roi'][3] - bbox_state_idy['face_info']['roi'][1] > options.face_size_threshold[1]):
                            bool_add_face = True
                    ...
                    if len(list(lpr_kind_np)) and len(list(lpr_kind_np[lpr_score_np > options.capture_lpr_score_threshold])) > 0:
                        ...
                        bbox_info_idx['plate_info']['kind_stabel'] = capture_lpr_kind
                    if len(list(lpr_num_np)) and len(list(lpr_num_np[lpr_score_np > options.capture_lpr_score_threshold])) > 0 :
                        ...
                        bbox_info_idx['plate_info']['num_stabel'] = capture_lpr_num
                    
                    if self.country_type == "morocco":
                        lpr_city_np = np.array(bbox_state_idy['state']['lpr_city_list'])

                        if len(list(lpr_city_np)) and len(list(lpr_city_np[lpr_score_np > options.capture_lpr_score_threshold])) > 0 :
                            capture_lpr_city, capture_lpr_city_frame = Counter(list(lpr_city_np[lpr_score_np > options.capture_lpr_score_threshold])).most_common(1)[0] 
                            bbox_info_idx['plate_info']['city_stabel'] = capture_lpr_city
            ...
            if is_new_id_bool:
                ...
                if self.demo_type == "lpr":
                    ...
                    if options.sort_type == "face":
                        ...
                        bbox_state_idy['state']['stable_loc'] = bbox_info_idx['person_info']['roi']
            ...
            for _, bbox_state_idy in self.params_dict['bbox_state_container'].items():
                ...
                if bbox_state_idy['track_id'] == capture_id_idx:
                    ...
                    if self.demo_type == "lpr":
                        ...
                        car_attri_np = np.array(bbox_state_idy['state']['car_attri_list'])
                        ...
                        if len(lpr_num_np[lpr_score_np > options.capture_lpr_score_threshold]):
                            ...
                            if len(list(car_attri_np)):
                                capture_car_attri, capture_car_attri_frame = Counter(list(car_attri_np)).most_common(1)[0]
                            ...
                            if self.country_type == "china":
                                ...
                                if capture_lpr_num_frame >= options.capture_lpr_num_frame_threshold and \
                                    ...
                                    # car
                                    capture_res_dict['car_info']['attri'] = capture_car_attri
                                    ...
                                    if (capture_lpr_color == "blue" and len(capture_lpr_num) == 7) or \
                                        (capture_lpr_color == "green" and len(capture_lpr_num) == 8) or \
                                        (capture_lpr_color != "blue" and capture_lpr_color != "green"):
                            ...
                            elif self.country_type == "us" or self.country_type == "eu" or self.country_type == "chile":
                                if capture_lpr_num_frame >= options.capture_lpr_num_frame_threshold and \
                                    len(capture_from_container_list) and \
                                    capture_lpr_num not in self.params_dict['capture_res_container']:
                                ...
                                if capture_lpr_kind_frame >= options.capture_lpr_kind_frame_threshold and \
                                    ...
                                    # 基于规则的后处理策略，过滤部分误报
                                    if capture_res_dict['plate_info']['country'] == "dubai":
                                        if capture_res_dict['plate_info']['color'] == "red":
                                            capture_res_dict['plate_info']['color'] = "none"
                                        if capture_res_dict['plate_info']['color'] == "yellow" and capture_res_dict['plate_info']['kind'] != "":
                                            capture_res_dict['plate_info']['color'] = "none"
                                        if capture_res_dict['plate_info']['color'] == "green" and capture_res_dict['plate_info']['kind'] != "":
                                            capture_res_dict['plate_info']['color'] = "none"
                                ...
                                if capture_lpr_kind_frame >= options.capture_lpr_kind_frame_threshold and \
                                    capture_lpr_num_frame >= options.capture_lpr_num_frame_threshold and \
                                    capture_lpr_city_frame >= options.capture_lpr_city_frame_threshold and \
                                    len(capture_from_container_list) and \
                                    capture_lpr_num not in self.params_dict['capture_res_container'] and \
                                    capture_lpr_num != "none":
                            ...
                            elif self.country_type == "huanwei" or self.country_type == "mexican":
...
def split_plate_ocr(ocr):
    # 匹配前1-2位数字
    m = re.match(r"(\d{1,2})([a-z])(\d+)", ocr)
    if m:
        kind = m.group(1)
        city = m.group(2)
        num = m.group(3)
        return kind, city, num
    else:
        return "", "", ""
    
def split_plate_ocr_city(ocr):
    # 匹配只有字母和数字的情况
    m = re.match(r"([a-z^])(\d+)", ocr)
    if m:
        city = m.group(1)
        num = m.group(2)
        return "", city, num
